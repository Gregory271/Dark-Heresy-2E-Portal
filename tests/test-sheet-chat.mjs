...
const styles = readFileSync(new URL("../foundry-module/dh2-portal/styles/dh2-portal.css", import.meta.url), "utf8");
assert.match(styles, /\.dh2-roll-card--success\s*\{[^}]*#58d187/s);
assert.match(styles, /\.dh2-roll-card--failure\s*\{[^}]*#ef665f/s);
assert.match(styles, /:has\(\.dh2-roll-card\) \.dice-roll \{ display: none; \}/);
const messages = [];
let dice = [32];
let blind = false;
globalThis.foundry = {};
...
assert.match(messages.at(-1).flavor, /dh2-roll-card--success/);
assert.match(messages.at(-1).flavor, /<strong>SUCCESS<\/strong><span>3 DEGREES OF SUCCESS<\/span>/);
assert.match(messages.at(-1).flavor, /<dt>Rolled<\/dt><dd>32<\/dd>/);
assert.match(messages.at(-1).flavor, /<dt>Target<\/dt><dd>50<\/dd>/);
...
assert.match(messages.at(-1).flavor,/<dt>Potential hits<\/dt><dd>2<\/dd>/);
...
assert.match(messages.at(-1).flavor, /dh2-roll-card--failure/);
dice = [9, 4];
await rollSheetDice(actor, { quantity: 2, sides: 10, damage: { keep: 1, primitive: 7, modifier: 3 } });
assert.match(messages.at(-1).flavor, /dh2-roll-card--damage/);
assert.match(messages.at(-1).flavor, /<strong>DAMAGE<\/strong><span>10 RAW DAMAGE<\/span>/);
