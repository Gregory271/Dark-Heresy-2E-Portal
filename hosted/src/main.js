...
function renderReview() {
  ...
  const renderKnownSkillRow = (record, interactive = false) => {
    ...
    return `<button type="button" class="review-skill-test" data-skill-row data-skill-category="trained" data-skill-search="${escapeHtmlAttribute(`${displayName} ${skill.characteristic} ${rankNames[rank - 1]}`.toLowerCase())}" data-roll-review-skill="${skill.id}" data-skill-speciality="${escapeHtmlAttribute(speciality)}" aria-label="${escapeHtmlAttribute(`Roll ${displayName}, target ${target}. ${tooltip}`)}" ${tooltip ? `title="${escapeHtmlAttribute(tooltip)}"` : ""}><span class="review-skill-label">${escapeHtmlAttribute(displayName)}</span><span>${rankNames[rank - 1]} · ${skill.characteristic} target <strong>${target}</strong></span></button>`;
  };
